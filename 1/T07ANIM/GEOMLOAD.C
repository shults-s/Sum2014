...
*/
static VOID LoadMaterials( ef2GEOM *G, CHAR *FileName )
{
  FILE *F;
  ef2MATERIAL DefMat, Mat;
...
BOOL EF2_GeomLoad( ef2GEOM *G, CHAR *FileName )
{
  ...
  size = 
    ...
    sizeof(ef2UV) * vtn +                     /*   текстурные координаты vtn */
  ...
  EF2_DefaultColor = ColorSet(1, 1, 1);
  for (p = 0; p < pn; p++)
  {
    INT minv, maxv, j;
    ef2PRIM prim;
    ...
    EF2_PrimCreate(&prim, EF2_PRIM_TRIMESH, vn, fn * 3);
    ...
    EF2_GeomAddPrim(G, &prim);
  }
  /* освобождаем память из-под прочитанных данных */
  free(ReadV);
  return TRUE;
} /* End of 'EF2_GeomLoad' function */
