...
*/
static VOID LoadMaterials( ss3GEOM *G, CHAR *FileName )
{
  FILE *F;
  ss3MATERIAL DefMat, Mat;
...
BOOL SS3_GeomLoad( ss3GEOM *G, CHAR *FileName )
{
  ...
  size = 
    ...
    sizeof(ss3UV) * vtn +                     /*   текстурные координаты vtn */
  ...
  SS3_DefaultColor = ColorSet(1, 1, 1);
  for (p = 0; p < pn; p++)
  {
    INT minv, maxv, j;
    ss3PRIM prim;
    ...
    SS3_PrimCreate(&prim, SS3_PRIM_TRIMESH, vn, fn * 3);
    ...
    SS3_GeomAddPrim(G, &prim);
  }
  /* освобождаем память из-под прочитанных данных */
  free(ReadV);
  return TRUE;
} /* End of 'SS3_GeomLoad' function */
